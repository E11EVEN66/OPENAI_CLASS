...
async function generateMealPlan() {
  ...
  const dietaryRestrictions = await ask(
    "Do you have any allergies? (e.g., vegetarian, gluten-free) If none, type 'no': "
  );
  const preferredCuisine = await ask(
    "What is your preferred cuisine? (e.g., Italian, Asian, Mexican): "
  );

  const mealPlanPrompt = `
        Given the dietary restrictions: ${
          dietaryRestrictions === "no" ? "none" : dietaryRestrictions
  ...
  const mealPlanResponse = await gptPrompt(mealPlanPrompt, {
    max_tokens: 1500,
    temperature: 0.5,
  });
  ...
  const shoppingListResponse = await gptPrompt(shoppingListPrompt, {
    max_tokens: 800,
    temperature: 0.5,
  });
  ...
  const budgetResponse = await gptPrompt(budgetPrompt, {
    max_tokens: 1000,
    temperature: 0.5,
  });
